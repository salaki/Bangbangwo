...
$Paging=new Paging();
$html = '';
$select_linkedin=mysql_fetch_array(mysql_query("SELECT linkedin,about_me FROM members WHERE id=".$_SESSION['userid']));
if($select_linkedin['linkedin']=='' || $select_linkedin['about_me']==''){
    $checkSts=mysql_num_rows(mysql_query("SELECT * FROM sts_check where user_id=".$_SESSION['userid']." AND type='0'"));
    if($checkSts<=0){
        $value_bid=1;
    }
}
		//$getTopListQry = "SELECT DATE_FORMAT(a.taskdate, '%d-%M-%Y') DATEONLY, DATE_FORMAT(a.taskdate,'%h:%I %p') TIMEONLY,a.id,a.taskto,a.taskfrom,a.taskname,a.taskdate,a.taskduration,a.taskdescription,a.tasklocation,a.userid,a.tasktype,people,a.other FROM tasks as a LEFT JOIN bids as b ON a.id=b.task_id WHERE (a.status='1'  or a.status ='2')".$search." group by a.id order by a.id desc";
$getTopListQry = "SELECT * FROM (SELECT DATE_FORMAT(a.taskdate, '%d-%M-%Y') DATEONLY, DATE_FORMAT(a.taskdate,'%h:%I %p') TIMEONLY,a.id,a.taskto,a.taskfrom,a.taskname,a.taskdate,a.taskduration,a.taskdescription,a.tasklocation,a.userid,a.tasktype,people,a.other,a.taskmulti 
                    FROM tasks as a 
                    LEFT JOIN bids as b ON a.id=b.task_id 
                    LEFT JOIN members c ON a.userid=c.id
                    WHERE  (a.status='1'  or a.status ='2')".$search." AND c.university_id='".$_SESSION['university']."' group by a.id order by a.id DESC)  as t WHERE  NOT EXISTS (SELECT * FROM  bids a WHERE a.task_id=t.id AND a.tasker_id=".$_SESSION['userid'].")";
		...
		if(mysql_num_rows($getRecentQry)>0)
		{
			...
			{
				/*$total = mysql_num_rows(mysql_query("SELECT * FROM bids WHERE task_id ='".$row['id']."' AND tasker_id='".$_SESSION['userid']."'"));
				if($total == 0)
				{*/
				$taskTotal+=1;
				$bidStatus = '<img src="images/ajax-loader.gif" id="bid-img'.$row['id'].'" style="display:none;">  &nbsp;<a class="btn btn-primary" id="a-bid'.$row['id'].'" href="javascript:void(0)" onclick="bid('.$row['id'].',\''.$value_bid.'\')">Bid</a>';
				...
				//if($chkBid == 0)
				{
					$html .= '<div id="bid'.$row['id'].'"><hr>
						...
						$html .='<h4 class="hgr1">'.$row['taskname'].' - '.ucwords($row['tasktype']).'</h4>';
			if($row['taskfrom'] != '')
			{
