...
//            if ($row['status'] != '2')
  //              $edit = '<a class="btn btn-primary" href="edit_task.php?id=' . $row['id'] . '">Edit</a>';
            ...
            if ($deleteRecord > 0) {
                ...
                $edit = '<a class="btn btn-primary" href="edit_task.php?id=' . $row['id'] . '">Edit</a>';  /*============= edit by sandeep ==========   (22-08-2015)*/
                $delete = "Delete ";  /*============= edit by sandeep ==========   (22-08-2015)*/
            }
	 if($row['status']==4){
		$delete = "Delete";
		$edit='';
		  ...
		  <!-- /.row -->';
            ...
            if ($getRecord > 0) {
                ...
                $bidQuery = mysql_query("SELECT a.task_id,a.tasker_id,a.amount,a.message,a.status,b.fname,b.lname,b.image,b.likes,b.dislikes,b.linkedin,b.contact,b.email,b.about_me FROM bids as a LEFT JOIN members as b ON a.tasker_id=b.id WHERE task_id='" . $row['id'] . "'");
            }
	...
	<a onclick="satisfiedAction5(' . $bidResult['task_id'] . ',' . $bidResult['tasker_id'] . ',0,' . $PageNo . ')" href="javascript:void(0)" id="dislike' . $bidResult['tasker_id'] . '" class="btn btn-primary">Disatisfied</a>
		...
		<input type="hidden" value="' . $PageNo . '" id="transferdispageno">';
		  ...
		  <hr><div class="row" style="overflow:hidden">
            ...
            $chksatisfied1 = mysql_query("select * from satisfiedstatus where bid_id='" . $row['id'] . "' AND status='0' ");

            $edit = '';
            if ($row['status'] != '2')
                $edit = '<a class="btn btn-primary" href="edit_task.php?id=' . $row['id'] . '">Edit</a>';

            $deleteRecord1 = mysql_num_rows(mysql_query("SELECT * FROM bids where task_id='" . $row['id'] . "' and status!='3'"));
            if ($deleteRecord1 > 0) {
                if ($row['refund'] == '1' && $row['refund'] != '') {

                    $delete = 'Refunded';
                    $removebutton = "show";
                } else if (mysql_num_rows($chksatisfied1) > 0) {
                    $delete = "Delete";
                } else {
                    $delete = "Cancel ";
                }
