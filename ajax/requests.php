...
if (isset($_GET['done'])) {
    ...
    <script>
        $(document).ready(function () {
            ...
            setTimeout(function () {
                window.location.href = location + "?page_no=" + pageno;
            }, 1000);
        });
...
if (isset($_GET['page_no'])) {
    ...
    <script>
        $(document).ready(function () {
            pagingAjax(this, 'ajax/requests.php?page&page=<?php echo $_GET['page_no']; ?>', 'showTopId');
        });
    </script>
    <?php
}
if (isset($_GET['satisfied'])) {
    ...
    <script>
        $(document).ready(function () {
            var location = 'http://' + window.location.hostname + '/dashboard';
            var taskerid = <?php echo $_SESSION['tid']; ?>

            var taskid = <?php echo $_SESSION['bid']; ?>

            // alert(taskid + "|" + taskerid);

            var action = $("#transfersataction").val();

            var pageno = <?php echo $_GET['page']; ?>
            //alert(pageno);

            satisfiedAction(taskid, taskerid, action, pageno);
            setTimeout(function () {
                window.location.href = location;
                //pagingAjax(this, 'ajax/requests.php?page&page='+pageno, 'showTopId');
            }, 1000);
        });
...
<script>
    ...
    function satisfiedAction4(taskid, teskerid, def, page)
    {

        $("#trans_" + taskid + "_" + teskerid).click();
...
if (isset($_SESSION['bid']) || isset($_SESSION['tid']) || isset($_SESSION['amount'])) {
    unset($_SESSION['bid']);
    unset($_SESSION['tid']);
    unset($_SESSION['amount']);
}
$html = '';
// 2 is for finshed task and 3 is for deleted task.
    ...
    while ($row = mysql_fetch_array($getRecentQry)) {
        ...
        if ($row['taskmulti'] == '0') {
		  ...
		  <div class="row" id="con1" style="margin:0;">
            ...
            $deleteRecord = mysql_query("SELECT * FROM bids where task_id='" . $row['id'] . "' AND status NOT IN('3','4') LIMIT 1");
            ...
            if (mysql_num_rows($deleteRecord) > 0) {
                ...
                if ($row['refund'] == '1' && $row['refund'] != '') {
                    $removebutton = 'show';
                    $delete = 'Refunded';
                } else if (mysql_num_rows($chksatisfied) > 0 || $getData['status'] == 4) {
                    $delete = "Delete ";
                } else {
                    $delete = "Cancel";
                }
            ...
            if ($removebutton != "") {
                //$chkrefundinmultiple = mysql_query("select * from bids where task_id='".$row['id']."'");



                $rmbutton = '<a class="btn btn-primary" href="javascript:void(0);" onclick="taskAction(\'task\',' . $row['id'] . ',\'removeTask\')">Delete </a>';
                $actionbutton = '<div class="con6"><img src="images/ajax-loader.gif" id="remove-img' . $row['id'] . '" style="display:none;"> ' . $edit . '  ' . $rmbutton . '</div>';
            } else if ($delete == 'Cancel') {
                $actionbutton = '<div class="con6"><img src="images/ajax-loader.gif" id="remove-img' . $row['id'] . '" style="display:none;"> ' . $edit . '  <a class="btn btn-primary" href="javascript:void(0)" onclick="taskAction(\'task\',' . $row['id'] . ',\'CancelTask\')">' . $delete . '</a></div>';
            } else {
...
//echo $bidQuery; exit();
            ...
            while ($bidResult = mysql_fetch_array($bidQuery_1)) {
                ...
                // Status 4 is for rejected bid
                if ($bidResult['status'] != 4) {
                    $abc++;
...
<script>
  ...
  $(window).on("popstate", function() {
    handler.close();
        ...
        <input type="submit" id="trans_' . $taskid . '_' . $taskerid . '" name="submit" class="btn btn-primary" value="Payment"> </form>
        ...
        ';
			...
			<div id="" class="col-md-12">
			  ...
			  <div class="col-xs-8" id="white1">
				...
				<p>Contact : ' . $bidResult['contact'] . '</p><p>Email : ' . $bidResult['email'] . '</p>';
                    ...
                    if ($row['refund'] == '1') {
                        $html.='<div class="rhtyi"> <img src="images/ajax-loader.gif" id="hire-img' . $bidResult['tasker_id'] . '" style="display:none;"> &nbsp;<a href="javascript:void(0)" class="btn btn-primary" style="background-color:#3071A9;color:#fff;">Refunded</a> </div>';
                    } else {
                    ...
                    if ($bidResult['about_me'] != '') {
                        ...
                        $html.='<p>&nbsp;</p>
                                        ...
                                        <p ><b>About me: </b>
                                         <p style="text-align: left; float: left; width: 90%;" id="about_p_' . $a . '">' . substr($bidResult['about_me'], 0, 200) . '</p>
                                            </div>
		  ...
		  <div class="row" id="con1" style="margin:0;">
            ...
            $edit = '';
            $rejected_bids = mysql_query("SELECT * FROM bids where task_id='" . $row['id'] . "' AND status ='4'");
            $chksatisfied = mysql_query("select * from satisfiedstatus where bid_id='" . $row['id'] . "' and status = '1'");
            $bidsquery = mysql_query("Select * from bids where task_id = '" . $row['id'] . "'");

            $total_bids = mysql_num_rows($bidsquery);

            $total_rejected_bids = mysql_num_rows($rejected_bids);
            $refunded_bids = mysql_query("SELECT * FROM bids where task_id='" . $row['id'] . "' AND refund ='1'");
            $total_refunded_bids = mysql_num_rows($refunded_bids);

            $total_satisfy_rejected_bids = intval($total_refunded_bids) + mysql_num_rows($chksatisfied);

            // Sachin Changes here
            $bid_button = TRUE;
            switch ($row['status']) {
                case '1':
                    if ($total_bids == 0 || $total_rejected_bids == $total_bids) {
                        $task_button = "edit/delete";
                        $bid_button = FALSE;
                    } elseif ($total_satisfy_rejected_bids == $total_bids) {
                ...
                default :
                    if ($total_bids == 0) {
                        $task_button = "edit/delete";
                        $bid_button = FALSE;
                    } else if ($total_bids != 0 && $total_rejected_bids == $total_bids) {
                        $task_button = "delete";
                    } else {
                        $task_button = "delete";
                    }
            ...
            switch ($task_button) {
                ...
                case 'cancel':
                    $actionbutton = '<div class = "con6"><img src = "images/ajax-loader.gif" id = "remove-img' . $row['id'] . '" style = "display:none;"> ' . $edit . ' <a class = "btn btn-primary" href = "javascript:void(0)" onclick = "taskAction(\'task\',' . $row['id'] . ',\'CancelTask\')">Cancel</a></div>';
                    break;
            ...
            $bidQuery = mysql_query("SELECT a.task_id,a.tasker_id,a.amount,a.message,a.status,b.fname,b.lname,b.image,b.likes,b.dislikes,b.linkedin,b.contact,b.email,b.about_me FROM bids as a LEFT JOIN members as b ON a.tasker_id=b.id WHERE task_id='" . $row['id'] . "'");  // 22-08-2015 	
            ...
            <a class = "btn btn-primary" id = "a-remove' . $bidResult['tasker_id'] . '" href = "javascript:void(0)" onclick = "rejectSingleBid(' . $bidResult['tasker_id'] . ',' . $bidResult['task_id'] . ',' . $PageNo . ')">Reject</span></a>';
                        ...
                        /* $hireStatus = '<img src = "images/ajax-loader.gif" id = "hire-img'.$bidResult['tasker_id'].'" style = "display:none;"> &nbsp;
                          <a class = "btn btn-primary" id = "a-hire'.$bidResult['tasker_id'].'" href = "javascript:void(0)" onclick = "hireTask('.$bidResult['tasker_id'].','.$bidResult['task_id'].',\'hireBid\','.$PageNo.')">Hire </span></a> <a class = "btn btn-primary" id = "a-remove'.$bidResult['tasker_id'].'" href = "javascript:void(0)" onclick = "rejectSingleBid('.$bidResult['tasker_id'].','.$bidResult['task_id'].','.$PageNo.')">Reject</span></a>';
                         */

                        $amnt = $bidResult['amount'] * 100;

                        $hireStatus = '<img src = "images/ajax-loader.gif" id = "hire-img' . $bidResult['tasker_id'] . '" style = "display:none;"> &nbsp;
            <a style = "float:right;margin-left:14px;" class = "btn btn-primary" id = "a-remove' . $bidResult['tasker_id'] . '" href = "javascript:void(0)" onclick = "rejectSingleBid(' . $bidResult['tasker_id'] . ',' . $bidResult['task_id'] . ',' . $PageNo . ')">Reject</span></a> &nbsp;
        ...
        $(".customButton").on("click", function(e) {
            ...
            handler.open( {
                ...
                <a onclick="satisfiedAction5(' . $bidResult['task_id'] . ', ' . $bidResult['tasker_id'] . ', 0, ' . $PageNo . ')" href="javascript:void(0)" id="dislike' . $bidResult['tasker_id'] . '" class="btn btn-primary">Disatisfied</a>
                <div class="con6"><img src="images/ajax-loader.gif" id="remove-img' . $bidResult['task_id'] . '" style="display:none;"> ' . $edit . '  <a class="btn btn-primary" href="javascript:void(0)" onclick="taskAction(\'task\',' . $bidResult['task_id'] . ',\'CancelTask\',\'' . $bidResult['tasker_id'] . '\')">Cancel</a></div>
                <input type="hidden" value="' . $bidResult['task_id'] . '" id="transferdistaskid">
                <input type="hidden" value="' . $bidResult['tasker_id'] . '" id="transferdistaskerid">
                <input type="hidden" value="0" id="transferdisaction">
                <input type="hidden" value="' . $PageNo . '" id="transferdispageno">';
                ...
                <div class="row" style="overflow:hidden">
                    ...
                    if ($bidResult['about_me'] != '')
                        $html.='<p>&nbsp;</p><p ><b>About me: </b>
                                <div style="float:left;width:100%"><p style="text-align: left;  float: left;" id="about_p_' . $a . '">' . substr($bidResult['about_me'], 0, 200) . '</p>
                                </div>
                                <!--<a target="_blank" href="/about_me.php?id=' . $bidResult['tasker_id'] . '">Read More</a>--></p>';
                    $html.='</div>';


                    $refunded = mysql_query("select * from bids where task_id='" . $row['id'] . "' && tasker_id='" . $bidResult['tasker_id'] . "'");
                    $refundedrow = mysql_fetch_array($refunded);
                    if ($refundedrow['refund'] == '1') {
                        $html.='<div class="rhtyi"> <img src="images/ajax-loader.gif" id="hire-img' . $bidResult['tasker_id'] . '" style="display:none;"> &nbsp;<a href="javascript:void(0)" class="btn btn-primary" style="background-color:#3071A9;color:#fff;">Refunded</a> </div>';
                    } else {
                        $html.='<div class="rhtyi"> ' . $hireStatus . ' </div>';
                    }
