...
if(isset($_REQUEST['action']) && $_REQUEST['action'] == 'removeTask')
{
	...
	$query= mysql_query("SELECT * FROM tasks WHERE id='".$_REQUEST['taskid']."' and  id NOT IN (SELECT bid_id FROM satisfiedstatus ) ORDER BY  `id` DESC");
	// task refund  
	if(mysql_num_rows($query)>0)
	{
	        $cquery= mysql_query("select * from stripepayment where taskid = '".$_REQUEST['taskid']."'");
	        ...
	        $amnt= mysql_query("select * from bids where task_id = '".$_REQUEST['taskid']."'");
	       ...
	       echo $error;
			...
			$data  = mysql_query("select * from members where id='".$chargeid['taskerid']."'");
