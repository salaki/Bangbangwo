...
$getTopListQry = "SELECT DATE_FORMAT(taskdate, '%d-%M-%Y') DATEONLY, DATE_FORMAT(taskdate,'%H:%I %p') TIMEONLY,id,taskname,taskfrom,taskto,tasklocation,taskdescription,taskduration,tasktype,people,other,status,taskmulti FROM tasks WHERE userid='".$_SESSION['userid']."' and status!='3' order by id desc";
	...
	while($row = mysql_fetch_array($getRecentQry))
	 {
		...
		if($row['taskmulti'] == '0')
		{
		  ...
		  while($bidResult = mysql_fetch_array($bidQuery))
		  {
                         ...
                         $query = mysql_query("select * from stripepayment where taskid ='".$bidResult['task_id']."' && taskerid ='".$bidResult['tasker_id']."' && status='1'");
			...
			{
			$publishablekey = mysql_query("select * from stripecredentials where id ='1'");
			$presult = mysql_fetch_array($publishablekey);
			...
			$hireStatus.='<form action="charge.php" method="POST" style="float:left">
		                <script 
		                    ...
		                    data-key="'.$presult['publishablekey'].'"
		                    data-name="Shinedezign"
		                    data-description="Test"
		                    data-amount="'.$amnt.'">
		                </script>
		                 <input type="hidden" name="taskid" value="'.$bidResult['task_id'].'">
		                 <input type="hidden" name="taskerid" value="'.$bidResult['tasker_id'].'">
		                  <input type="hidden" name="amount" value="'.$amnt.'">
	                </form> <a class="btn btn-primary" id="a-remove'.$bidResult['tasker_id'].'" href="javascript:void(0)" onclick="rejectSingleBid('.$bidResult['tasker_id'].','.$bidResult['task_id'].','.$PageNo.')">Reject</span></a>'; 		
			...
			if($bidResult['status'] == 1)
			{
				...
				$transfer = mysql_query("SELECT * FROM satisfiedstatus WHERE bid_id='".$bidResult['task_id']."' AND tasker_id='".$bidResult['tasker_id']."'");
				$datatransfer =mysql_fetch_array($transfer);
	...
	<a onclick="satisfiedAction('.$bidResult['task_id'].','.$bidResult['tasker_id'].',0,'.$PageNo.')" href="javascript:void(0)" id="dislike'.$bidResult['tasker_id'].'" class="btn btn-primary">Disatisfied</a>';
				...
				//echo $find = mysql_num_rows($likeQuery);
					...
					if($datatransfer['paymentstatus']==0)
					{
					        $amnt = $bidResult['amount']*100;
...
<form action="transfer.php" method="post" style="float:right;">
<input type="hidden" name="amount" value="'.$amnt.'" >
