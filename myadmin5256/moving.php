...
if(isset($_REQUEST['startdate']))
{
	...
	$getTopListQry = "SELECT DATE_FORMAT(added_date,'%Y-%m-%d') as dt FROM `tasks` where tasktype='movinghelp' and status!='0' and added_date>='".$date."' and added_date<='".$endDate."'";
}
else
{
	$getTopListQry = "SELECT DATE_FORMAT(added_date,'%Y-%m-%d') as dt FROM `tasks` where tasktype='movinghelp' and status!='0' and added_date>='".$date."'";
}
...
$html='<div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <ol class="breadcrumb">
                            <li class="active">
                                 <a href="/admin/moving.php?type=2">All</a> | <a href="?type=0">Last 7 Days</a> | <a href="?type=1">1 Month</a>  
                            </li>
                        </ol>
                    </div>
                </div>
...
if(mysql_num_rows($getRecentQry)>0)
{
	$html.='<table class="table table-bordered table-hover table-striped">
		 ...
		 foreach($finalArr as $date=>$count){
					$countAct = mysql_result(mysql_query("SELECT count(*) FROM `tasks` where tasktype='movinghelp'  and date(added_date) = '".$date."'"),0);
					$countPrfrm = mysql_result(mysql_query("SELECT count(*) as prfrm FROM `tasks` where tasktype='movinghelp' and status='2' and date(added_date) = '".$date."'"),0);
