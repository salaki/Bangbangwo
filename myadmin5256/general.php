...
$finishedPosts = mysql_result(mysql_query("SELECT count(id) FROM tasks WHERE status='2' and tasktype='general'"),0);
...
$requestTasks = mysql_result(mysql_query("SELECT count(id) FROM tasks  WHERE status='1' and tasktype='general'"),0);
...
{
	$taskType = 'Tasks Requests - general';
}
else
{
	$taskType = 'Tasks Performed - general';
}

if(!isset($_GET['type']) || $_GET['type'] <=1)
{
	$where = ' where a.tasktype="general" and a.status="1" ';
	
}
else
{
	$where = ' where a.tasktype="general" and a.status="2" and a.status="'.$_GET['type'].'"';
}
if(isset($_POST['search']))
{
	$where = ' where a.tasktype="general"  and b.email like "'.$_POST['search'].'%"';
}
if(isset($_POST['searchtxt']))
{
	$where = ' where a.tasktype="general" and a.taskname like "'.$_POST['searchtxt'].'%"';
}*/
...
if(isset($_REQUEST['startdate']))
{
	...
	$getTopListQry = "SELECT DATE_FORMAT(added_date,'%Y-%m-%d') as dt FROM `tasks` where tasktype='general' and status!='0' and added_date>='".$date."' and added_date<='".$endDate."'";
}
else
{
	$getTopListQry = "SELECT DATE_FORMAT(added_date,'%Y-%m-%d') as dt FROM `tasks` where tasktype='general' and status!='0' and added_date>='".$date."'";
}
                    ...
                    <div class="col-lg-12">
						...
						<div class="col-lg-4">
							<div class="input-group">
								...
								<div>From : <input type="text" id="datepicker"  name="startdate" placeholder="yyyy-mm-dd" class="form-control" value="'.$_POST['startdate'].'"></div>
								
								<div>To : <input type="text" id="datepicker1"  name="enddate" placeholder="yyyy-mm-dd" class="form-control" value="'.$_POST['enddate'].'"></div>
                        ...
                        <ol class="breadcrumb">
                            <li class="active">
                                 <a href="/admin/general.php?type=2">All</a> | <a href="?type=0">Last 7 Days</a> | <a href="?type=1">1 Month</a>  
                            </li>
                        </ol>
                    </div>
                </div>
	...
	$html.='<table class="table table-bordered table-hover table-striped">
		...
		while($date<=$endDate){
		 ...
		 foreach($finalArr as $date=>$count){
					$countAct = mysql_result(mysql_query("SELECT count(*) FROM `tasks` where tasktype='general'  and date(added_date) = '".$date."'"),0);
					$countPrfrm = mysql_result(mysql_query("SELECT count(*) as prfrm FROM `tasks` where tasktype='general' and status='2' and date(added_date) = '".$date."'"),0);
