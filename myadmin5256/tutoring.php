...
$finishedPosts = mysql_result(mysql_query("SELECT count(id) FROM tasks WHERE status='2' and tasktype='tutoring'"),0);
...
$requestTasks = mysql_result(mysql_query("SELECT count(id) FROM tasks  WHERE status='1' and tasktype='tutoring'"),0);
...
{
	$taskType = 'Tasks Requests - tutoring';
}
else
{
	$taskType = 'Tasks Performed - tutoring';
}

if(!isset($_GET['type']) || $_GET['type'] <=1)
{
	$where = ' where a.tasktype="tutoring" and a.status="1" ';
	
}
else
{
	$where = ' where a.tasktype="tutoring" and a.status="2" and a.status="'.$_GET['type'].'"';
}
if(isset($_POST['search']))
{
	$where = ' where a.tasktype="tutoring"  and b.email like "'.$_POST['search'].'%"';
}
if(isset($_POST['searchtxt']))
{
	$where = ' where a.tasktype="tutoring" and a.taskname like "'.$_POST['searchtxt'].'%"';
}*/
...
if(isset($_REQUEST['startdate']))
{
	...
	$getTopListQry = "SELECT DATE_FORMAT(added_date,'%Y-%m-%d') as dt FROM `tasks` where tasktype='tutoring' and status!='0' and added_date>='".$date."' and added_date<='".$endDate."'";
}
else
{
	$getTopListQry = "SELECT DATE_FORMAT(added_date,'%Y-%m-%d') as dt FROM `tasks` where tasktype='tutoring' and status!='0' and added_date>='".$date."'";
}
...
$getRecentQry = $Paging->showRecord($getTopListQry,'taskdate desc') or die(mysql_error());
$html='<div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        ...
                        <ol class="breadcrumb">
                            <li class="active">
                                 <a href="/admin/tutoring.php?type=2">All</a> | <a href="?type=0">Last 7 Days</a> | <a href="?type=1">1 Month</a>  
                            </li>
                        </ol>
                    </div>
                </div>
	...
	$html.='<table class="table table-bordered table-hover table-striped">
		...
		while($date<=$endDate){
		 ...
		 foreach($finalArr as $date=>$count){
					...
					$countAct = mysql_result(mysql_query("SELECT count(*) FROM `tasks` where tasktype='tutoring' and date(added_date) = '".$date."'"),0);
					$countPrfrm = mysql_result(mysql_query("SELECT count(*) as prfrm FROM `tasks` where tasktype='tutoring' and status='2' and date(added_date) = '".$date."'"),0);
